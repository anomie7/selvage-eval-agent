...
본 문서는 [`02-commit-collection.md`](../specs/02-commit-collection.md) 명세를 기반으로 한 실제 구현 코드입니다.
...
class CommitCollector:
    ...
    NON_CODE_FILES = {
        ...
        'package-lock.json', '.lock', '.cache'
    }
    
    MINOR_PENALTY_FILES = {
        '.json', '.yaml', '.yml', '.toml', '.ini', '.env', 
        '.md', '.mdc', 'Dockerfile', 'Makefile', 'requirements.txt'
    }
    ...
    def collect_commits(self) -> MeaningfulCommitsData:
        ...
        for repo_config in self.repo_configs:
            ...
            try:
                ...
                filtered_commits = self._filter_commits(commits, self.commit_filters)
                scored_commits = [self._score_commits(commit) for commit in filtered_commits]
                selected_commits = self._select_top_commits(
                    scored_commits, 
                    repo_config.commits_per_repo or self.commit_filters.commits_per_repo
                )
    ...
    def _parse_commit_stats(self, stats_output: str) -> CommitStats:
        ...
        for line in lines:
            if 'file changed' in line or 'files changed' in line:
                ...
                # 파일 수
                if 'file' in parts[0]:
                    files_changed = int(re.search(r'(\d+)', parts[0]).group(1))
    ...
    def _filter_commits(self, commits: List[CommitData], filters: CommitFilters) -> List[CommitData]:
        ...
        for commit in commits:
            # 1. 통계 기준 필터링
            if not self._passes_stats_filter(commit, filters):
                continue
    ...
    def _passes_stats_filter(self, commit: CommitData, filters: CommitFilters) -> bool:
        """통계 기준 필터링"""
        stats = commit.stats
        
        # 파일 수 범위 확인
        if not (filters.min_files <= stats.files_changed <= filters.max_files):
            return False
        
        # 최소 변경 라인 수 확인
        if stats.total_lines_changed < filters.min_lines:
            return False
...
```python
    ...
    def _execute_git_command(self, command: str, cwd: str) -> ToolResult:
        """Git 명령어 실행"""
        try:
            return self.tool_executor.execute_tool("execute_safe_command", {
                "command": command,
                "cwd": cwd,
                "timeout": 60
            })
...
from selvage_eval.config import load_config
from selvage_eval.tools.tool_executor import ToolExecutor
...
def main():
    ...
    # 설정 로드
    config_path = "selvage-eval-config.yml"
    ...
    try:
        # 커밋 수집 실행
        results = collector.collect_commits()
        
        # 결과 저장
        output_path = Path("selvage-eval-results/meaningful_commits.json")
        output_path.parent.mkdir(parents=True, exist_ok=True)
        
        results.save_to_json(str(output_path))
        
        # 요약 정보 출력
        print(f"\n=== 커밋 수집 완료 ===")
        print(f"처리된 저장소 수: {len(results.repositories)}")
        print(f"총 선별된 커밋 수: {results.total_commits}")
        print(f"결과 저장 위치: {output_path}")
        
        for repo in results.repositories:
            print(f"\n저장소: {repo.repo_name}")
            print(f"  - 전체 커밋: {repo.metadata.total_commits}")
            print(f"  - 필터링 후: {repo.metadata.filtered_commits}")
            print(f"  - 선별된 커밋: {repo.metadata.selected_commits}")
            print(f"  - 처리 시간: {repo.metadata.processing_time_seconds:.2f}초")
            
            if repo.commits:
                scores = [c.score.total_score for c in repo.commits]
                print(f"  - 점수 범위: {min(scores)} ~ {max(scores)}")
        
    except Exception as e:
        logging.error(f"커밋 수집 실패: {e}")
        raise
...
commit_filters:
  ...
  max_files: 10
  min_lines: 50
  commits_per_repo: 8  # 기본값
```

이 구현은 명세 문서의 모든 요구사항을 충족하며, 프로덕션 환경에서 즉시 사용 가능한 완전한 코드입니다.